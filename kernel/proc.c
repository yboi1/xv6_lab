...
// =============释放内核页表
void 
freeprocpagetable(pagetable_t pagetable)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){    // 非子页表
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
      freeprocpagetable((pagetable_t)child);
      pagetable[i] = 0;
    } else if(pte & PTE_V){   // 子页表
      pagetable[i] = 0;
      // panic("freeprocpagetable: leaf");
    }
  }
...
proc_freekernelpagetable(struct proc*p)
{
  ...
  freeprocpagetable(p->k_pagetable);
}
